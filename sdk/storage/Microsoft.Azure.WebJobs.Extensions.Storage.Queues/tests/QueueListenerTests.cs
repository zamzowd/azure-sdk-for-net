...
namespace Microsoft.Azure.WebJobs.Extensions.Storage.Queues
{
    public class QueueListenerTests : LiveTestBase<WebJobsTestEnvironment>
    {
        ...
        public void GetMessageReceiveCount_DynamicConcurrencyEnabled_ReturnsExpectedValue()
        {
            ...
            mockConcurrencyThrottleManager.Setup(p => p.GetStatus()).Returns(new ConcurrencyThrottleAggregateStatus { State = ThrottleState.Disabled });
            ...
            int result = localListener.GetMessageReceiveCount(out TaskSeriesCommandResult delay);
            Assert.AreEqual(1, result);
        }

        [TestCase]
        public void GetMessageReceiveCount__ReturnsExpectedValue()
        {
            int result = _listener.GetMessageReceiveCount(out TaskSeriesCommandResult delay);
            Assert.AreEqual(_mockQueueProcessor.Object.QueuesOptions.BatchSize, result);
            Assert.AreEqual(default(TaskSeriesCommandResult), delay);
        }
