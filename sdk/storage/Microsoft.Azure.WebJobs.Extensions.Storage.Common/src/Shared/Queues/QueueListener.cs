...
namespace Microsoft.Azure.WebJobs.Extensions.Storage.Common.Listeners
{
    internal sealed partial class QueueListener : IListener, ITaskSeriesCommand, INotificationCommand, IScaleMonitor<QueueTriggerMetrics>
    {
        ...
        public async Task<TaskSeriesCommandResult> ExecuteAsync(CancellationToken cancellationToken)
        {
            ...
            using (HttpPipeline.CreateClientRequestIdScope(clientRequestId))
            {
                try
                {
                    ...
                    {
                        int numMessagesToReceive = GetMessageReceiveCount(out TaskSeriesCommandResult delay);
        ...
        internal int GetMessageReceiveCount(out TaskSeriesCommandResult delay)
        {
            delay = default(TaskSeriesCommandResult);
            ...
            if (_concurrencyManager != null && _concurrencyManager.Enabled)
            {
                ...
                // based on the current degree of parallelism for this function (AvailableInvocationCount).
                var concurrencyStatus = _concurrencyManager.GetStatus(_functionId);
                numMessagesToReceive = Math.Min(concurrencyStatus.AvailableInvocationCount, QueuesOptions.MaxBatchSize);
                if (numMessagesToReceive == 0)
                {
                    // if we're not healthy or we're at our limit, we'll wait
                    // a bit before checking again
                    delay = CreateDelayResult(concurrencyStatus.NextStatusDelay);
                }
            }
